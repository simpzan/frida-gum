...
function getFunctionMaps() {
    // runCmd('./trace-cmd/tracecmd/trace-cmd report -tR | grep trace-cmd-8500 > ./trace.txt');
    // runCmd('./trace-cmd/tracecmd/trace-cmd report -f > ./fns.txt');
    const out = fs.readFileSync('./fns.txt', 'utf8');
    const functions = {};
    out.split('\n').forEach(line => {
        const parts = line.split(' ');
        functions[parts[0]] = parts;
    });
    return functions;
}
...
async function processLineByLine(file) {
  ...
  for await (const line of rl) {
    ...
    const ts = parseFloat(m[3]) * 1000000;
    const phase = m[4];
    const fnAddr = m[5];
